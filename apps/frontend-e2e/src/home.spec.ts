...
test.describe('frontend home journeys', () => {
  test('shows the landing experience', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /loan management system/i }).first()).toBeVisible();
    await expect(page.getByText(/track applications, disbursements, repayments/i)).toBeVisible();
  ...
  test('supports language and theme preferences', async ({ page }) => {
    ...
    await expect.poll(async () => page.locator('html').getAttribute('lang')).toBe('hi');
  ...
  test('opens a loan category detail page', async ({ page }) => {
    ...
    await page.getByRole('link', { name: /retail loan management systems/i }).click();

    await expect(page).toHaveURL(/loan-types\/retail-loan-management-systems/);
    await expect(page.getByRole('heading', { name: /retail loan management systems/i })).toBeVisible();
    await expect(page.getByText(/banks, nbfcs, and housing finance providers/i)).toBeVisible();
  });
});
